...
int AtenderProtocoloHTTP(int sd)
{
    ...
    if(bLeidos < 0) {
		fprintf(stderr, "ERROR: [%s] error leyendo del socket\n",strerror(errno));
		return -1;
	}
...
int ObtenerSocket(int puerto)
{
    ...
    log("LISTEN: indicamos que atienda hasta %d conexiones simultaneas",DEFAULT_MAX_CLIENTS);
...
void LanzarServicio(int puerto)
{
    ...
    if(sfdServidor==-1) {
		fprintf(stderr, "ERROR: no se ha podido establecer al conexion\n");
		exit(-1);
	}
    ...
    while (1) {
        ...
        switch (fork()) {
            ...
            case  0:    // El hijo atiende la nueva solicitud
				log("PROCESO HIJO %d: atendiendo proceso de aplicacion en socket %d",(int)getpid(),sfdCliente);
                close(sfdServidor);
...
int main()
{
	...
	//lanzando el servicio que este en el puerto indicado
	log("Lanzamos servicio en puerto: %d",DEFAULT_PORT);
	LanzarServicio(DEFAULT_PORT);
   
	//en realidad nunca llegaremos aqui
	return 1;
}	/* de main */
